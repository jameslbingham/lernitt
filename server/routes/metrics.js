...
// Admin Metrics API — mock-first, contract-aligned (ESM)
// Keeps existing /health route and adds /growth, /lessons, /financials, /riskops.

import express from "express";
const router = express.Router();

// -------- Config (mock toggle) --------
const MOCK = String(process.env.VITE_MOCK ?? process.env.METRICS_MOCK ?? "1") === "1";
...
router.get("/growth", async (req, res) => {
  if (MOCK) {
    ...
    // Optional: conversion by source (placeholder)
    ...
    const conversionBySource = sources.map((s) => {
      ...
      return { source: s, signups: su, firstBookings: fb, rate: Math.round((fb / su) * 1000) / 10 };
    });
...
router.get("/lessons", async (req, res) => {
  if (MOCK) {
    ...
    const distCounts = weighted([35, 22, 14, 9, 6, 5, 3, 2, 2, 1, 1]); // 1..10, 11+
    ...
    for (let day = 0; day < 7; day++) {
      for (let hour = 0; hour < 24; hour++) {
        const base = [19, 20, 21].includes(hour) ? rand(35, 70) : rand(4, 28);
        heatmapHourDay.push({ hour, day, count: base });
      }
    }

    // Type breakdown
    const tb = weighted([18, 76, 6]); // trial/paid/group
    const typeBreakdown = [
    ...
    const durationVariance = months.map((m) => {
      const booked = rand(38, 46) * 10; // 380–460
...
router.get("/financials", async (req, res) => {
  if (MOCK) {
    ...
    const revenueByCategory = categories.map((c) => ({ category: c, revenue: rand(8000, 42000) }));
    const revenueByCountry = countries.map((c) => ({ country: c, revenue: rand(6000, 38000) }));
    const arpuTrend = months.map((m) => ({ month: m, arpu: rand(42, 86) }));
    const revenuePerTutorTrend = months.map((m) => ({ month: m, revenuePerTutor: rand(180, 520) }));
    ...
    const avgPriceByCategory = categories.map((c) => ({ category: c, avgPrice: rand(12, 45) }));
    const commissionTrend = months.map((m) => ({ month: m, commissionPct: rand(15, 22) / 100 }));
    const refundRateTrend = months.map((m) => ({ month: m, rate: rand(1, 6) / 100 }));
    const chargebackRateTrend = months.map((m) => ({ month: m, rate: rand(0, 3) / 100 }));
...
router.get("/riskops", async (req, res) => {
  if (MOCK) {
    ...
    const errorTypes = ["auth.failed", "payment.error", "booking.conflict", "email.bounce"];
